...
class FlocSample(object):
    def __init__(self, Df_list, t, nframes, params, percentile):
...
class FlocData(object):
    ...
    def get_flocstats_table(self, percentile, method='all', **kwargs):
        
        # returns all flocs as a single sample
        if method=='all':
            Dflist = []
            for i in range(len(self.tframes)):
                Dflist.extend(self.frames[i].tolist())
            
            Dflist = np.array(Dflist)
            
            floc_samples = [FlocSample(Dflist, self.tframes[0], len(self.tframes),
                                     self.params, percentile),]
        
        elif method=='frame':
            Df_samples = []
            for i in range(len(self.tframes)):
                floc_samples.append(FlocSample(self.frames[i], self.tframes[i], 1,
                                             self.params, percentile))
        ...
        # returns irregularly spaced samples that have minimum number of flocs
            ...
            floc_samples = []
            for i in range(len(self.tframes)):
                tlist.append(self.tframes[i])
                Dflist.extend(self.frames[i].tolist())
                if len(Dflist) >= kwargs['minflocs']:
                    floc_samples.append(FlocSample(Dflist, tlist[0], len(tlist), 
                                                   self.params, percentile))
        ...
        fdict = {'datetime': [samp.t for samp in floc_samples],
